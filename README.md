...
PhoneCamRedux is a high-performance, multi-platform virtual webcam system. It allows using your Android or iOS phone as a high-fidelity virtual webcam on Windows (DirectShow virtual camera) or macOS (CMIO Camera Extension).

The system features an ultra-low latency 4K60 UDP production streaming path, stateful bandwidth adaptation, loss recovery via NACK, playout jitter buffering, a multiplexed USB cable wired fallback, and a legacy RTSP fallback.

## Key Features

- **Multi-Platform Support:**
  - **Senders:** Android (Camera2, MediaCodec HEVC) and iOS (AVFoundation, VideoToolbox HEVC).
  - **Receivers:** Windows (DirectShow Softcam) and macOS (CoreMedia I/O Camera Extension).
- **High-Performance 4K60 HEVC Protocol:**
  - **RTP over UDP:** Low-overhead media transport on port `5004` (RFC 7798).
  - **Stateful ABR (Adaptive Bitrate):** Dynamic bitrate and resolution adaptation (10% steps over 5-second observation window).
  - **Loss Recovery (NACK):** Selective retransmission requests with a 256-packet ring buffer.
  - **Jitter Buffer:** Playout smoothing with configurable delay (default `50 ms`).
- **USB Cable Wired Fallback:**
  - Length-prefixed TCP multiplexing on port `47822` (via ADB/usbmuxd) for lossless, zero-jitter streaming with `0 ms` jitter buffer target.
- **RTSP Legacy Fallback:**
  - Legacy RTSP-over-TCP fallback streaming on port `8554` for broad compatibility.
- **Auto-Discovery & Pairing:**
  - UDP beacons on port `47821` transmit stream parameters and a six-digit pairing code to secure the connection on local Wi-Fi.
- **Robust Orientation Handling:**
  - Locked landscape streaming, output-rotation corrections, and support for foldable screen geometries.
...
```bash
# Build device target
xcodebuild -workspace ios/PhoneCam.xcworkspace -scheme PhoneCam -sdk iphoneos -configuration Release BUILD_DIR=build/ios

# Run simulator unit tests
xcodebuild test -workspace ios/PhoneCam.xcworkspace -scheme PhoneCamTests -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 14'
```
...
No-argument startup waits up to 15 seconds for a PhoneCam LAN discovery beacon and connects to the first discovered phone. Manual URL fallback:
...
- Softcam is MIT.
- FFmpeg must be distributed according to the exact build configuration. Use LGPL dynamic builds for the intended MIT-friendly package.
...
See [docs/verification-report.md](docs/verification-report.md) for exact commands, outputs, emulator artifacts, and remaining host-specific checks.
